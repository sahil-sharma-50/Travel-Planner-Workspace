...
from app.core.prompts import PREFERENCES_PROMPT, SYSTEM_PROMPT
...
def run_agent_stream(
    ...
    if interests:
        system_prompt += PREFERENCES_PROMPT.format(
            user_name=user_name, interests=interests
        )
    ...
    yield json.dumps(
        {
            ...
            "reason": "Understanding your travel query",
        }
    ) + "\n"
    ...
    if tool_calls:
        ...
        for tool_call in tool_calls:
            ...
            step_descriptions = {
                ...
                "change_theme": f"Change theme to {function_args.get('theme', 'unknown')}",
            }
            plan_steps.append(
                step_descriptions.get(function_name, f"Call {function_name}")
            )

        has_theme_tool = any(tc.function.name == "change_theme" for tc in tool_calls)
        suffix = "task completed" if has_theme_tool else "Generate recommendation"
        plan_text = " → ".join(plan_steps) + f" → {suffix}"
        ...
        available_functions = {
            ...
            "change_theme": change_theme_tool,
        }
        ...
        for tool_call in tool_calls:
            ...
            reasons = {
                ...
                "change_theme": "Updating application theme",
            }
            yield json.dumps(
                {
                    "type": "status",
                ...
                yield json.dumps(
                    {
                        "type": "client_action",
                        "action": "change_theme",
                        "data": {"theme": function_args.get("theme")},
                    }
                ) + "\n"
        ...
        yield json.dumps(
            {
                ...
                "reason": "Creating personalized recommendation",
            }
        ) + "\n"
        ...
        yield json.dumps(
            {"type": "client_action", "action": "update_travel_form", "data": form_data}
        ) + "\n"
        ...
        for tool_call in tool_calls:
            ...
            if function_name == "get_weather":
                ...
                if city_key in attr_router.ATTRACTIONS:
                    structured_data["attractions"] = attr_router.ATTRACTIONS[city_key][
                        "attractions"
                    ]
        ...
        if (
            ...
            or structured_data["shows"]
            or structured_data["weather"]
        ):
            yield json.dumps(
                {"type": "structured_data", "content": structured_data}
            ) + "\n"
            ...
            yield json.dumps(
                {
                    ...
                    "data": {
                        "attractions": [
                            item["name"] for item in structured_data["attractions"]
                        ],
